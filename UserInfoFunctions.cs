...
{
    private readonly ILogger<UserInfoFunctions> _logger;
    private readonly IHttpContextAccessor _httpContextAccessor;

    public UserInfoFunctions(ILogger<UserInfoFunctions> logger, IHttpContextAccessor httpContextAccessor)
    {
        _logger = logger;
    ...
    public async Task<string> GetCurrentUser(
        [McpToolTrigger("get_current_user", "Get current logged-in user information from Microsoft Graph using Azure App Service authentication and On-Behalf-Of flow.")]
        ToolInvocationContext context)
    {
        ...
        var headers = httpTransport.Headers;
            ...
            var errorOutput = new
            {
                ...
                message = $"Error during token exchange and Graph API call: {ex.Message}. " +
                         (!string.IsNullOrEmpty(hostname) 
                             ? $"You're logged in but might need to grant consent to the application. Open a browser to the following link to consent: https://{hostname}/.auth/login/aad?post_login_redirect_uri=https://{hostname}/api/authcomplete"
